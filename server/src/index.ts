...
async function squareCheckout(request: Request, env: Env) {
  ...
  const base = env.SQUARE_ENVIRONMENT === 'Production' ? 'https://connect.squareup.com' : 'https://connect.squareupsandbox.com'; const response = await fetch(`${base}/v2/online-checkout/payment-links`, { method: 'POST', headers: { authorization: `Bearer ${env.SQUARE_ACCESS_TOKEN}`, 'content-type': 'application/json', 'square-version': '2026-08-19' }, body: JSON.stringify({ idempotency_key: key, payment_note: `kurabu:${account.id}`, quick_pay: { name: `KurabuPoker ${interval === 'monthly' ? '月額' : '年額'}会員`, price_money: { amount: planPrice(interval), currency: 'JPY' }, subscription_plan_id: variationId } }) }); const result = await response.json<{ payment_link?: { url?: string; id?: string }; errors?: unknown }>(); if (!response.ok || !result.payment_link?.url) return json({ error: 'square_checkout_failed', details: result.errors }, 502); return json({ checkoutUrl: result.payment_link.url, paymentLinkId: result.payment_link.id, interval, amountJpy: planPrice(interval) }, 201)
}
...
const planPrice = (interval: string) => interval === 'annual' ? 14400 : interval === 'monthly' ? 1500 : 0
