...
function provider(provider: 'google' | 'line') { const redirect = `${window.location.origin}/api/auth/${provider}/callback`; window.location.href = `/api/auth/${provider}/start?redirect_uri=${encodeURIComponent(redirect)}` }
</script>
<template>
